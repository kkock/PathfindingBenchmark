...
import { Heuristic, euclideanHeuristic, getWeightedHeuristic } from '../services/Heuristic'
...
import { prepareSuites, runSuites, SerializedBenchmarkResult } from "../benchmark/Suite"
...
export function runConfig (configPath: string): void {
  ...
  for (const heuristic of config.services.get(Heuristic)!) {
    for (const cost of config.services.get(Cost)!) {
      for (const algorithm of config.algorithms) {
        ...
        for (const opts of optsSets) {
          ...
          for (const [scenName, scenDefs] of scenFiles) {
            ...
            const result = runSuites(suites, [algorithm], services, opts)
